 ...
 */
 function AutoComplete (options) {
     this.config = {
        ...
        url                : '/yrzx/home/ajax_search_name',   // url接口
...
//              });
              ...
              $(item).keyup(function(e){
                  ...
                  var targetVal = $.trim($(this).val()),
                      ...
                      elemWidth = $(this).outerWidth() + 122;  //宽度+外边距+dl宽度-border
...
//                     $(this).val('');
          ...
          var stylesheet = '.auto-tips { margin: 0 1px; list-style: none;height:auto !important;padding: 0px;position:absolute; border:1px solid #ccc; top:27px; left:0; z-index:999; width:100%;background:#fff !important;}' + 
                           '.auto-tips p {overflow: hidden;margin: 1px 0;padding: 5px 5px 5px 55px; color: #666;text-decoration: none;line-height: 23px;white-space: nowrap;cursor: pointer;zoom: 1;}' + 
                           '.auto-tips p img{ vertical-align:middle;float:left;}' + 
                           '.create-input{line-height:39px; width:595px; heigt:62px}' + 
                           '.create-input span{height:39px;float:left; margin-left:54px;display:block; width:454px;}' +
                           '.create-input span i,.auto-tips span a{font-style:normal;float:left;cursor:default; font-size:18px;color:#bbb}' +
                           '.create-input span a{padding:0 8px 0 3px;cursor:pointer; font-size:18px;}' +
                           '.auto-tips p.hoverBg {background-color: #4ad89c;color: #fff;cursor: pointer;}' + 
                           '.hidden {display:none;}';
    ...
    _doPostAction: function(targetVal,targetParent) {
        ...
        $.ajax({
            ...
            dataType: "json",
            async: true,
            data: {keyword: targetVal},
            success: function (data) {
                if(data.status == 1){
                    results = [data.result];
                    self._renderHTML(results[0],targetParent);
                    self._executeClick(results[0],targetParent);
                }
            }
        });
        // 设置假数据， 假如返回的数据如下：
//        var results = [{lastName:'wilson',emplId:'E09871'},{lastName:'wilson2',emplId:'E0988'},{lastName:'wilson3',emplId:'E0989'}];
...
//                    '</p>';
            html += '<p  data-html = "'+ret[i].company+'" class="p-index'+i+'">' + 
                   //    '<img src="'+ret[i].image+'" style="margin-right:5px;" height="25" width="25" title="" alt="">' + 
                       '<span>'+ret[i].company+'</span>' + 
                    '</p>';
        }
    ...
    _hover: function(targetParent){
        $('.create-input span',targetParent).hover(function(){
            $(this).css({"background":'#fff','padding-left':'0px'}); 
        },function(){
             $(this).css({"background":''});
        });
    },
    ...
    _createDiv: function(targetParent,dataAttr){
         ...
         $('.alink',targetParent).css({'float':'right','background':'none','text-decoration':'none'});
    },
